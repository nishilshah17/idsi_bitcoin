...
/*  def printToFile(f: java.io.File)(op: java.io.PrintWriter => Unit) {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val blockFilePath = "hdfs://" + args(0) + "/*-blocks-r-00000"
    val transactionFilePath = "hdfs://" + args(0) + "/*-transactions-r-00000"
    //local output path: /Users/NishilShah/workspace/idsi_bitcoin/sample_output
    val outputFilePath = "hdfs://" + args(1) + "/new_addresses"
    ...
    val addressesPerMonth = addressFirstSeen.map(entry => (entry._2, 1)).reduceByKey(_ + _)
      .repartition(1).saveAsTextFile(outputFilePath)
  }
}
