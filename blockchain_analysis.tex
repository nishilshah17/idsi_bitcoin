...
MapReduce is an adequate method for this task because the blockchain can easily be split into equally sized, independent chunks. The processing of one block does not rely on another and thus can be parallelized. We start by parsing each block's byte contents in the map phase to \lstinline{Block} and \lstinline{Transaction} objects. The mapper outputs all blocks and transactions as values by using a parent \listinline{GenericWritable} class. We use a date-based key to split the work among several reducers. Each reducer will receive a list of transactions and blocks that occurred in the same month and output them to separate files. The MapReduce output directory looks something like this:
